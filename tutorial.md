...
## Setting expectations: Searching vs. filtering
Before we begin, it's important that we set a baseline for what we're looking to accomplish in this tutorial, because search, as you may know, is a real can of worms. In fact, you might even call it a kettle of fish.
...
We'll spare you the trouble of populating those values for 100 records, so now is a good time to import the `import.sql` file into your database, so that we have some data to search on. It populates the `AvailableStart` and `AvailableEnd` columns with a random date between now (April 2015) and a year from now (April 2016). The end date is a random value of 1-14 days after the start date.

If you're doing this tutorial beyond that range of time, you may want to update the database manually using the following queries:

```sql
UPDATE Property SET AvailableStart = FROM_UNIXTIME(
        UNIX_TIMESTAMP(NOW()) + FLOOR(0 + (RAND() * 31536000))
);
UPDATE Property SET AvailableEnd = FROM_UNIXTIME(
...
The page that renders the search results for property is pretty distinct. In the attached assets for this lesson, you'll find the HTML for a new template in `property-search-results.html`. Let's import that into our project.

Copy the contents of the file into *themes/one-ring/templates/Layout/PropertySearchPage.ss*.

Then, create a new class for the Page in *mysite/code/PropertySearchPage.php*
...
*themes/one-ring/templates/Layout/PropertySearchPage.ss* (line 38)
```html
<% loop $Results %>
