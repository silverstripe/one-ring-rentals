...
The most common misconception about using extensions is that they can overload methods like subclasses. This is _not the case._ Let's say you want to update the `logIn()` method of the `Member` class so that it pings a thirdparty service, so you write something like this:
